...
const IspuPage = () => {
  ...
  const basicEmpty = Object.values(basicMap).every((v) => v === false);
  const advEmpty = Object.values(advMap).every((v) => v === false);
  ...
  const handleClick = (key) => {
    if (advMap[key]) {
      navigate(`/forecast/advanced/${key}`);
    } else {
      navigate(`/forecast/basic/${key}`);
    }
  };
            ...
            <div className="col-12 col-md-4 col-lg-3" key={i}>
              <div
                ...
                <div className="card-body d-flex flex-column justify-content-center align-items-center py-4">
                  ...
                  {advMap[p.key] && (
                    <span className="badge bg-primary mt-2">Advanced</span>
                  )}
                  {!advMap[p.key] && basicMap[p.key] && (
                    <span className="badge bg-success mt-2">Basic</span>
                  )}
                </div>
              </div>
            </div>
