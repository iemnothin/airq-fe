...
const DashboardPage = () => {
  ...
  const restartBackend = async () => {
    ...
    try {
      const res = await fetch("https://api-airq.abiila.com/api/v1/status/restart", {
        method: "POST",
        headers: {
          admin_key: "AirQ-Admin-2025", // KEY UNTUK AUTORIZASI
        },
      });
  ...
  const backendMap = {
    ...
    degraded: { label: "Degraded", color: "#ffc107", icon: <FaExclamationTriangle /> },
  ...
  return (
    <div className="container py-4 animate__animated animate__fadeIn">
      ...
      <div className="mb-4 text-center text-md-start">
        ...
        <p className="text-muted mb-0">Ringkasan status sistem dan teknologi yang digunakan.</p>
      </div>

      {/* STATUS SYSTEM */}
      <Card className="mb-4 shadow-sm border-0 rounded-4">
        <Card.Body>
          <h5 className="fw-bold text-primary mb-3 text-center">Status Sistem</h5>
          ...
          {loading ? (
            ...
            <div className="text-center text-danger py-3">❌ Tidak dapat memuat status sistem.</div>
          ) : (
            <Row className="g-4 justify-content-center">
              {/* BACKEND */}
              <Col md={4} sm={6}>
                <Card className="shadow-sm border-0 rounded-4 p-4 text-center h-100">
                  <div style={getStatusStyle(backendUI.color)} className="mx-auto">
                    {backendUI.icon} {backendUI.label}
                  </div>
                  <h6 className="fw-bold text-dark mt-3 mb-2">Backend</h6>
                  <p className="small text-muted mb-0">
                    CPU {status.cpu_usage} · RAM {status.ram_usage} · {latency} ms
                  </p>
                </Card>
              </Col>
              ...
              <Col md={4} sm={6}>
                <Card className="shadow-sm border-0 rounded-4 p-4 text-center h-100">
                  <div
                    ...
                    className="mx-auto">
                    {status.database === "connected" ? <FaCheckCircle /> : <FaTimesCircle />}{" "}
              ...
              <Col md={4} sm={6}>
                <Card className="shadow-sm border-0 rounded-4 p-4 text-center h-100">
                  <div
                    ...
                    className="mx-auto">
                    {status.model_status === "ready" ? <FaCheckCircle /> : <FaTimesCircle />}{" "}
      ...
      {history.length > 0 && (
        <Card className="mb-4 shadow-sm border-0 rounded-4">
          <Card.Body>
            <h5 className="fw-bold text-primary mb-3 text-center">Resource Monitoring (Realtime)</h5>
            <Line
              data={{
      ...
      <Card className="shadow-sm border-0 rounded-4">
        <Card.Body>
          <h5 className="fw-bold text-primary mb-3">Teknologi yang Digunakan</h5>
          <Row className="g-3">
            {[
